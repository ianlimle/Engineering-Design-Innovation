...
################################## Define camera functions ############################################            
    def capture_plant(tier):
        if tier == 1:
            subprocess.call("fswebcam -d /dev/video1 -r 3280x2464 --no-banner -S0 vfarm/1.jpg",shell=True)         
            print("Captured tray: 1")
            subprocess.call("fswebcam -d /dev/video0 -r 3280x2464 --no-banner -S0 vfarm/2.jpg",shell=True) 
            print("Captured tray: 2")
            
        if tier == 2:
            subprocess.call("fswebcam -d /dev/video1 -r 3280x2464 --no-banner -S0 vfarm/3.jpg",shell=True) 
            print("Captured tray: 3")
            subprocess.call("fswebcam -d /dev/video0 -r 3280x2464 --no-banner -S0 vfarm/4.jpg",shell=True) 
            print("Captured tray: 4")
        
    def capture(tier):
        if tier == 1:
            MQTTClient.capture_plant(tier)
            f=open('vfarm/1.jpg', "rb")
            byteArray=base64.b64encode(f.read())
            MQTTClient.publish("vfarm/1", byteArray)
            time.sleep(1)
            MQTTClient.capture_plant(tier)
            f=open('vfarm/2.jpg', "rb")
            byteArray=base64.b64encode(f.read())
...
############################ Define line follower commands for arduino  ##########################################################################        
    ...
    client = mqtt.Client("robot_rpi", clean_session=True, userdata=None)
...
class RobotHarvest(MQTTClient):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.machine.add_transition(trigger= 'ReachedTier2',
                                    source= 'shiftinguptocamposition',
...
#########################################################################################     
    def ready_for_tier1_slider_release(self):
        ...
        for i in range(2):
             ...
             msgpayload = q.get()
             print(msgpayload)
             msgpayload_dict.update({msgpayload.keys(): msgpayload.values()})
